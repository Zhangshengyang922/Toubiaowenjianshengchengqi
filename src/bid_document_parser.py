...
class BiddingDocumentParser:
    ...
    def _extract_project_info(self):
        ...
        id_num = None
        id_patterns = [
            r'(?:项目编号|招标编号|采购编号|采购项目编号)[：:\s]*([A-Za-z0-9\u4e00-\u9fa5〔〕\[\]()（）\-_]+)',
            r'编号[：:\s]*([A-Za-z0-9\u4e00-\u9fa5〔〕\[\]()（）\-_]+)',
            r'(?:招标编号|项目编号).{0,3}([A-Za-z]+\d{4,}[A-Za-z0-9\u4e00-\u9fa5〔〕\[\]()（）\-_]*)',
            r'(?:招标编号|项目编号).{0,3}([A-Z]{2,}[A-Za-z0-9\u4e00-\u9fa5〔〕\[\]()（）\-_]{5,})',
        ]
    ...
    def _extract_dates(self):
        ...
        opening = None
        opening_pats = [
            # 竞争性磋商常用：开启\n时间：2026年7月18日（星期六）09时00分
            r'(?:开标|开启)(?:\s*时间)?\s*[：:]\s*(\d{4}\s*年\s*\d{1,2}\s*月\s*\d{1,2}\s*日?)',
            r'开标时间[：:\s]*(\d{4}\s*[年/\-]\s*\d{1,2}\s*[月/\-]\s*\d{1,2}\s*日?)',
        ...
        deadline = None
        deadline_pats = [
            r'(?:投标截止\s*(?:时间|日期|日?)|递交截止\s*(?:时间|日期|日?)|截止时间|提交响应文件截止时间)[：:\s]*(\d{4}\s*年\s*\d{1,2}\s*月\s*\d{1,2}\s*日?)',
            r'(?:投标截止\s*(?:时间|日期|日?)|递交截止\s*(?:时间|日期|日?)|截止时间)[：:\s]*(\d{4}\s*[年/\-]\s*\d{1,2}\s*[月/\-]\s*\d{1,2}\s*日?)',
            r'(\d{4}\s*年\s*\d{1,2}\s*月\s*\d{1,2}\s*日).{0,30}(?:截止|前|递交)',
        ]
